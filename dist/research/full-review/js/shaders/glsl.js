...
float edgeTransfer(float myMass, float myMu, float myEn, float myAggr,
  ...
  float drained = min(base, victimEn * 0.25); // at most four edges share the victim budget
...
void main(){
  ...
  if(uMetabolism > 0.5){
    ...
    float overflow = max(0.0, cEnergy - 3.0);
    cEnergy -= overflow; freeE += overflow; // return excess instead of deleting energy
  ...
  if(uGenome > 0.5){
    ...
    if(newMass > 0.02 && mu <= 0.001){
      ...
      mu = nbMu + (hash(vUV*71.0+uTime)-0.5)*uMut;
      sigma = uSigBase + (hash(vUV*37.0-uTime)-0.5)*uMut;
      aggr = clamp(aggr + (hash(vUV*17.0+uTime)-0.5)*uMut*7.5, 0.0, 1.0);
    }
    if(newMass > 0.02){
...
void main(){
  vec2 uv = (vUV - 0.5) * vec2(uCam.w, 1.0) / uCam.z + 0.5 + uCam.xy;
  uv = fract(uv);
