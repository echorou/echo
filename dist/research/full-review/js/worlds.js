...
export function makeWorld(name, W, H, opts) {
  ...
  switch (name) {
    ...
    case 'garden': {
      // Four matched, evenly separated bodies. Avoid random overlapping genomes
      // in a default preset; variation belongs to the explicit mutation control.
      for (const x of [0.25,0.75]) for (const y of [0.25,0.75])
        stampOrbium(s0,s1,W,H,(W*x)|0,(H*y)|0,MU,SIG,0,scale,0.2,0.8);
      break;
    }
    case 'collide': {
      // Two parallel bodies separated diagonally by half the periodic world.
      for (const v of [0.25,0.75])
        stampOrbium(s0,s1,W,H,(W*v)|0,(H*v)|0,MU,SIG,0,scale,0.2,0.8);
      break;
    }
    case 'soup': {
