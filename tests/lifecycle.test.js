...
test('restart resets manual parameters, speed and observation view',()=>{const {c,el}=harness();el('light').value='0';el('light').oninput();el('mutation').value='.015';el('mutation').oninput();el('speed').value='6';el('speed').onchange();el('view').value='4';el('restart').click();assert.equal(c.params.light,.35);assert.equal(c.params.mut,0);assert.equal(c.params.speed,1);assert.equal(el('view').value,'0');assert.equal(el('auto').checked,false);assert.equal(c.tick,0);});
...
test('baseline disables genetic experiments; evolution is explicit and restart returns to baseline',()=>{const {c,el}=harness();assert.equal(c.params.genome,false);assert.equal(el('mutation').disabled,true);assert.equal(el('auto').disabled,true);el('mode').value='evolution';el('mode').onchange();assert.equal(c.params.genome,true);assert.equal(el('mutation').disabled,false);assert.equal(el('auto').checked,false);el('auto').checked=true;el('auto').onchange();el('restart').click();assert.equal(c.params.genome,false);assert.equal(el('auto').checked,false);assert.equal(el('world').value,'glider');});
test('restoring a checkpoint pauses before advancing it',async()=>{const {c,el}=harness();await c.save();el('restart').click();c.restore();assert.equal(c.running,false);assert.equal(el('play').textContent,'Resume');});
test('manual single stepping checks depleted state immediately',()=>{const {c,el}=harness();c.engine.mass=0;el('step').click();assert.equal(c.running,false);assert.equal(el('world-health').hidden,false);});

test('terminal decline reaches the enabled caretaker instead of skipping feedback',()=>{const {c,el}=harness();el('mode').value='evolution';el('mode').onchange();el('auto').checked=true;el('auto').onchange();for(let i=0;i<180;i++)el('step').click();c.engine.mass=0;for(let i=0;i<30;i++)el('step').click();assert.equal(el('lessons').textContent,1);assert.ok(Number(el('reward').textContent)<0);});
